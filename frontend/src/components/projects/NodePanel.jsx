...
function NodePanel({ node, onUpdate, onClose, readOnly, mobile = false }) {
  ...
  return (
    <motion.aside initial={{ x: '100%', opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 26, stiffness: 320 }} className={`surface-panel ${mobile ? 'fixed inset-x-4 top-[max(72px,calc(var(--sat)+72px))] z-50 h-[70vh] w-auto rounded-2xl border' : 'h-full w-[340px] rounded-l-2xl rounded-r-none border-r-0 md:w-[390px]'}`}>
