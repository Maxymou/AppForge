  ...
  return (
    <div className={`flow-node ${selected ? 'flow-node--selected' : ''}`}>
      ...
      <div className="px-3 py-2.5 sm:px-4 sm:py-3">
        ...
        {items.length ? <div className="flow-node__items-divider mt-2.5 space-y-1.5 pt-2">{items.map((item, i) => <div key={`${item}-${i}`} className="relative pr-4"><Handle id={handleIdFromIndex(i)} type="source" position={Position.Right} style={{ top: 11 + i * 23, right: -4 }} /><p className="truncate text-[11px] text-slate-200/90 sm:text-xs"><span className="mr-1 text-indigo-300">•</span>{item}</p></div>)}</div> : null}
      </div>
