  ...
  return (
    <div className="select-none" style={{ paddingLeft: `${depth * 16}px` }}>
      <div className="tree-node__row group flex items-start gap-2 py-1.5">
        ...
        <div className={`tree-node__card ${depth === 0 ? 'tree-node__card--section' : ''} flex flex-1 items-start gap-2 px-3 py-2`}>
          ...
          <div className="flex-1">
            ...
            {isEditing ? <Input ref={editInputRef} value={editValue} onChange={(e) => setEditValue(e.target.value)} onBlur={async()=>{if(editValue.trim()) await updateNode(node.id,{title:editValue.trim()}); setIsEditing(false)}} className="py-1" /> : <button type="button" onDoubleClick={() => setIsEditing(true)} onClick={() => setShowEditor(true)} className={`tree-node__title ${depth === 0 ? 'tree-node__title--section' : ''} flex-1 text-left text-sm text-slate-200`}>{node.title}</button>}
          </div>
          {!isEditing && <div className="tree-node__actions flex shrink-0 items-center gap-1"><IconButton onClick={() => setShowEditor(true)} tooltip="Éditer" label="Éditer">✎</IconButton><IconButton onClick={() => setIsAddingChild(true)} tooltip="Ajouter un enfant" label="Ajouter un enfant">+</IconButton><IconButton onClick={() => setConfirmOpen(true)} tooltip="Supprimer" label="Supprimer" variant="danger">×</IconButton></div>}
        </div>
      </div>
